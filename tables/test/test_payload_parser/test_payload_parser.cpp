...
void test_parse_pending_status(void)
{
    OrderEvent evt;
    bool ok = parseStr(R"({"order":1,"status":"pending","eta":"15 min"})", evt);
    TEST_ASSERT_TRUE(ok);
    TEST_ASSERT_EQUAL_STRING("pending", evt.status);
}
...
{
    // status field has a 16-byte buffer (15 chars + null terminator).
    OrderEvent evt;
    bool ok = parseStr(R"({"status":"this_is_way_too_long_for_buffer"})", evt);
    TEST_ASSERT_TRUE(ok);
    TEST_ASSERT_EQUAL_size_t(15, std::strlen(evt.status));
    TEST_ASSERT_EQUAL_CHAR('\0', evt.status[15]); // null-terminated
}
...
int main(int argc, char **argv)
{
    ...
    RUN_TEST(test_parse_pending_status);
